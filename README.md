...
DIAG服务支持通过UDS诊断仪触发SEC服务申请证书流程：

- **RID**: 0xFF01 (CERTIFICATE_REQUEST)
- **子功能**: 0x01 (启动例程)
- **安全要求**: 需要安全访问解锁（0x27服务）
- **流程**: 生成密钥对 → 获取CSR → 提交CSR → 注入证书

**使用方法**:
1. 建立诊断会话（0x10 0x03）
2. 执行安全访问（0x27服务）
3. 发送证书申请请求（0x31 0x01 0xFF01）
4. 等待响应（可能返回NRC 0x78表示正在处理）
